...
export function activateInternal(context: vscode.ExtensionContext) {
    ...
    registerCommand('azureApiManagement.extractService', async (node: AzureTreeItem) => await extractApis(node));
    registerCommand('azureApiManagement.extractApi', async (node: ApiTreeItem) => await extractApis(node));
