...
import { extractApis, extractSingleApi, extractWithConfig } from './commands/extractAPIs';
...
export function activateInternal(context: vscode.ExtensionContext) {
    ...
    registerCommand('azureApiManagement.extractApis', async (node?: AzureTreeItem) => await extractApis(node));
    registerCommand('azureApiManagement.extractSingleApi', async (node?: ApiTreeItem) => await extractSingleApi(node));
    registerCommand('azureApiManagement.extractWithConfig', async (node?: AzureTreeItem) => await extractWithConfig(node));
